...
export function KenaiNetworkBanner() {
  return (
    <div className="border-b border-white/10 bg-white/60 backdrop-blur dark:bg-white/5">
      <div className="mx-auto flex max-w-7xl flex-col gap-4 px-4 py-4 sm:px-6 lg:flex-row lg:items-center lg:justify-between lg:px-8">
        ...
        <div className="flex flex-wrap gap-2">
          {networkLinks.map((site) => (
            <a key={site.href} href={site.href} target="_blank" rel="noreferrer" className="rounded-full border border-slate-300/70 bg-white/80 px-3 py-1 text-xs font-semibold text-slate-700 transition hover:border-alaska-ocean hover:text-alaska-ocean dark:border-white/10 dark:bg-white/5 dark:text-slate-200 dark:hover:text-white">
              {site.label}
            </a>
          ))}
        </div>
